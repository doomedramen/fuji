...
use fuji::security::integrity::{
    HashAlgorithm, IntegrityConfig, IntegrityStatus, IntegrityViolation,
    IntegrityViolationType, ViolationSeverity, ViolationStatus, RuntimeIntegrityChecker,
    ProcessInfo, MemoryRegion, IntegrityResponseConfig
};
...
async fn test_integrity_config_default_values() -> Result<()> {
    ...
    assert!(config.enable_code_integrity, "Code integrity should be enabled by default");
    assert!(config.enable_memory_integrity, "Memory integrity should be enabled by default");
    assert!(config.enable_data_integrity, "Data integrity should be enabled by default");
    assert_eq!(config.check_interval, 300, "Default check interval should be 300 seconds");
    assert_eq!(config.alert_threshold, 3, "Default alert threshold should be 3");
    assert_eq!(config.hash_algorithm, HashAlgorithm::Sha256, "Default hash algorithm should be SHA-256");
    assert!(!config.monitored_paths.is_empty(), "Should have default monitored paths");
    assert!(!config.critical_libraries.is_empty(), "Should have default critical libraries");
...
async fn test_hash_algorithms_consistency() -> Result<()> {
    ...
    assert_eq!(sha256_hash1.len(), 64, "SHA-256 should produce 64-character hex string");
    ...
    assert_eq!(sha512_hash1.len(), 128, "SHA-512 should produce 128-character hex string");

    // Verify different algorithms produce different hashes
    assert_ne!(sha256_hash1, sha512_hash1, "Different algorithms should produce different hashes");
...
async fn test_runtime_integrity_checker_creation() -> Result<()> {
    ...
    // Verify executable path is detected
    assert!(!checker.executable_path.as_os_str().is_empty());

    // Verify process info is populated
    ...
    println!("✓ Runtime integrity checker creation test passed");
    Ok(())
}
...
async fn test_library_path_detection() -> Result<()> {
    ...
    for library in common_libraries {
        if let Some(lib_path) = checker.find_library_path(library) {
            ...
            assert!(lib_path.to_string_lossy().contains(library), "Path should contain library name");
            found_count += 1;
        }
    }
    ...
    println!("✓ Library path detection test passed (found {}/{} libraries)", found_count, common_libraries.len());
    Ok(())
}
...
async fn test_violation_severity_levels() -> Result<()> {
    ...
    for (severity, expected_value) in severities {
        assert_eq!(severity as i32, expected_value,
                   "Severity level {:?} should have value {}", severity, expected_value);
    }
...
async fn test_code_integrity_violation_type() -> Result<()> {
    ...
    if let IntegrityViolationType::CodeModification { expected_hash, actual_hash, region: r } = violation_type {
...
async fn test_library_injection_violation_type() -> Result<()> {
    ...
    if let IntegrityViolationType::LibraryInjection { library_path, injection_method } = violation_type {
...
async fn test_control_flow_integrity_violation_type() -> Result<()> {
    ...
    if let IntegrityViolationType::ControlFlowViolation { expected_target, actual_target, function_name } = violation_type {
...
async fn test_memory_protection_violation_type() -> Result<()> {
    ...
    if let IntegrityViolationType::MemoryProtectionViolation { address, operation, protection_flags } = violation_type {
...
async fn test_runtime_hooking_violation_type() -> Result<()> {
    ...
    if let IntegrityViolationType::RuntimeHooking { function_name, hook_address, original_address } = violation_type {
...
async fn test_integrity_response_config() -> Result<()> {
    ...
    assert!(!config.enable_termination, "Termination should be disabled by default");
    assert!(!config.enable_core_dump, "Core dump should be disabled by default");
    assert!(config.enable_secure_shutdown, "Secure shutdown should be enabled by default");
    assert!(!config.alert_recipients.is_empty(), "Should have default alert recipients");
    assert!(config.custom_response_script.is_none(), "Custom script should be None by default");
    ...
    custom_config.alert_recipients.push("admin@company.com".to_string());
...
async fn test_integrity_checker_file_integrity() -> Result<()> {
    ...
    assert_eq!(hash1_2, hash2_2, "Hash should remain same for unchanged file");
...
async fn test_hash_algorithm_comparison() -> Result<()> {
    ...
    for (i, (_, hash_i)) in hashes.iter().enumerate() {
        for (j, (_, hash_j)) in hashes.iter().enumerate() {
            if i != j {
                assert_ne!(hash_i, hash_j, "Hashes from different algorithms should be unique");
            }
        }
    }

    // Verify SHA-512 produces longer hash than SHA-256
    let sha256_hash = hashes.iter().find(|(algo, _)| matches!(algo, HashAlgorithm::Sha256)).unwrap().1;
    let sha512_hash = hashes.iter().find(|(algo, _)| matches!(algo, HashAlgorithm::Sha512)).unwrap().1;
    assert!(sha512_hash.len() > sha256_hash.len(), "SHA-512 should produce longer hash than SHA-256");
...
async fn test_violation_context_manipulation() -> Result<()> {
    ...
    assert_eq!(context.get("request_path"), Some(&"/api/v1/mounts".to_string()));
...
async fn test_integrity_violation_serialization() -> Result<()> {
    ...
    assert_eq!(violation.source_process.pid, deserialized.source_process.pid);
...
async fn test_config_customization() -> Result<()> {
    ...
    assert!(config.monitored_paths.contains(&PathBuf::from("/custom/path")));
    assert!(config.critical_libraries.contains(&"custom_lib.so".to_string()));
...
async fn test_hash_performance() -> Result<()> {
    ...
    for algorithm in algorithms {
        ...
        assert!(duration.as_secs() < 1, "Hashing should complete in under 1 second");
    }
...
async fn test_hash_computation_stress() -> Result<()> {
    ...
    println!("✓ Hash computation stress test passed ({} iterations)", iterations);
    Ok(())
}
...
async fn test_large_file_hashing() -> Result<()> {
    ...
    assert!(duration.as_secs() < 5, "Large file hashing should complete in under 5 seconds");
