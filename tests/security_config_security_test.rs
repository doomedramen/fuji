...
use fuji::security::config_security::{
    ConfigSecurityManager, ConfigSecurityConfig, ConfigData, ConfigMetadata,
    UserPermissions, Permissions, LockType, BackupReason, ConfigOperation,
    ConfigOperationResult, ConfigHistoryEntry, ValidationSeverity, ValidationError,
    ValidationWarning, EncryptedConfig, ConfigBackup
};
...
async fn test_config_security_config_default_values() -> Result<()> {
    ...
    assert!(config.require_auth, "Authentication should be required by default");
    assert!(config.enable_backup, "Backup should be enabled by default");
    assert_eq!(config.backup_versions, 10, "Default backup versions should be 10");
    assert!(config.enable_validation, "Validation should be enabled by default");
    assert!(!config.strict_validation, "Strict validation should be disabled by default");
    assert!(config.enable_audit_logging, "Audit logging should be enabled by default");
    assert_eq!(config.file_permissions, 0o600, "Default file permissions should be 600");
    assert_eq!(config.dir_permissions, 0o700, "Default directory permissions should be 700");
    assert_eq!(config.max_file_size, 10 * 1024 * 1024, "Default max file size should be 10MB");
    assert!(config.allowed_extensions.contains("toml"), "TOML should be allowed");
    assert_eq!(config.lock_timeout, 300, "Default lock timeout should be 300 seconds");
    assert!(config.enable_rollback, "Rollback should be enabled by default");
...
async fn test_user_permissions_management() -> Result<()> {
    ...
    let has_read = manager.check_permissions("testuser", Permissions {
        read: true,
        ..Default::default()
    }).await?;
    assert!(has_read, "User should have read permission");

    let has_delete = manager.check_permissions("testuser", Permissions {
        delete: true,
        ..Default::default()
    }).await?;
    assert!(!has_delete, "User should not have delete permission");

    let has_admin = manager.check_permissions("testuser", Permissions {
        admin: true,
        ..Default::default()
    }).await?;
...
async fn test_configuration_locking() -> Result<()> {
    ...
    assert!(!lock_id.is_empty(), "Regular user should acquire write lock");
    ...
    let admin_lock_result = manager.acquire_lock(
        "test_config",
        "admin",
        LockType::Admin,
        "Admin lock attempt"
    ).await;
    assert!(admin_lock_result.is_err(), "Admin should not acquire lock on locked resource");

    // Release lock
    manager.release_lock("test_config", "regular", &lock_id).await?;
    ...
    let admin_lock_id = manager.acquire_lock(
        "test_config",
        "admin",
        LockType::Admin,
...
key_file = "/etc/fuji/key.pem""#.to_string(),
...
jwt_secret = "jwt_signing_key_very_long_string""#.to_string(),
        metadata: ConfigMetadata {
            name: "secrets.toml".to_string(),
            version: "1.0".to_string(),
            created_at: chrono::Utc::now(),
    ...
    let encrypted_data = manager.encrypt_config(&original_config, Some(encryption_key)).await?;
    assert!(encrypted_data.starts_with(b"FUJI_ENC"), "Encrypted data should have magic prefix");
    assert_ne!(encrypted_data.len(), original_config.content.len(), "Encrypted data should be different length");

    // Test decryption
    let decrypted_config = manager.decrypt_config(&encrypted_data, Some(encryption_key)).await?;
    assert_eq!(decrypted_config.content, original_config.content, "Decrypted content should match original");
    ...
    assert_eq!(decrypted_config.metadata.author, original_config.metadata.author);

    // Test decryption with wrong key
    let wrong_key_result = manager.decrypt_config(&encrypted_data, Some("wrong_key")).await;
    assert!(wrong_key_result.is_err(), "Decryption with wrong key should fail");
...
ssl = true"#;
    ...
    manager.restore_from_backup(&backup.backup_path, &restore_target).await?;
...
async fn test_configuration_history_tracking() -> Result<()> {
    ...
    assert_eq!(second_entry.previous_checksum, Some(first_entry.checksum.clone()));
...
new_feature = true"#.to_string();
    ...
    manager.rollback(&config_path, "rollback_admin", Some(1)).await?;

    // Verify rollback was logged
    let history = manager.get_history(Some(&config_path)).await?;
    assert_eq!(history.len(), 3, "Should have Create, Update, and Rollback entries");
...
async fn test_expired_lock_cleanup() -> Result<()> {
    ...
    let lock1_id = manager.acquire_lock(
        "config1",
        "lock_test_user",
        LockType::Write,
        "Testing lock 1"
    ...
    let lock2_id = manager.acquire_lock(
        "config2",
        "lock_test_user",
        LockType::Write,
        "Testing lock 2"
    ...
    let lock3_id = manager.acquire_lock(
        "config3",
        "lock_test_user",
        LockType::Read,
        "Testing lock 3"
    ...
    let release_result = manager.release_lock("config1", "lock_test_user", &lock1_id).await;
    assert!(release_result.is_err(), "Should not be able to release expired lock");
...
async fn test_file_size_validation() -> Result<()> {
    ...
    let load_result = manager.load_config(&large_config_path, "size_test_user", None).await;
...
async fn test_configuration_statistics() -> Result<()> {
    ...
    assert!(stats.last_operation.is_some(), "Should have last operation timestamp");
...
async fn test_permission_inheritance() -> Result<()> {
    ...
    let has_read = manager.check_permissions("unknown_user", Permissions {
        read: true,
        ..Default::default()
    }).await?;
    assert!(has_read, "Unknown user should inherit read permission");

    let has_write = manager.check_permissions("unknown_user", Permissions {
        write: true,
        ..Default::default()
    }).await?;
    assert!(!has_write, "Unknown user should not inherit write permission");
...
async fn test_concurrent_lock_operations() -> Result<()> {
    ...
    for i in 1..=3 {
        ...
        let handle = tokio::spawn(async move {
            manager_clone.acquire_lock(
                &format!("resource_{}", i),
                &format!("user{}", i),
                LockType::Write,
                &format!("Concurrent lock test {}", i)
            ).await
        });
        handles.push(handle);
    }
