...
fn test_mount_point_generation_safety() {
    ...
    for component in components {
        let _result = validator.sanitize_path_component(component);
        println!("Component '{}': {:?}", component, result);
    }

    // These should generate safe mount points even with malicious input
    let test_cases = vec![("nfs://server.com/../../../etc/passwd", "should fail")(
        "nfs://server.com/share/data",
        "should succeed",
    )("nfs://evil.com/../../root/.ssh", "should fail")(
        "nfs://good.com/export/data",
        "should succeed",
    )];
    ...
    for (url, expected) in test_cases {
        ...
        let _result = handler.generate_mount_point(url);
