...
use fuji::mount::drivers::{
    ...
    NfsHandler, SmbHandler, SshfsHandler,
};
...
fn test_command_injection_in_mount_options() {
    ...
    let malicious_options = vec![
        ...
        vec![
            "option1".to_string(),
            "option2|cat /etc/shadow".to_string()
        ], // Pipe injection
    ];
...
fn test_path_traversal_prevention() {
    ...
    for url in traversal_urls {
        let result = validator.validate_url(url);
        assert!(result.is_err(), "URL with path traversal should be rejected: {}", url);
    }
}
...
fn test_numeric_validation_in_options() {
    ...
    assert!(validator.validate_options("smb", &["uid=1000".to_string()]).is_ok());
    assert!(validator.validate_options("smb", &["gid=1000".to_string()]).is_ok());

    // Invalid numeric values should fail
    assert!(validator.validate_options("smb", &["uid=abc".to_string()]).is_err());
    assert!(validator.validate_options("smb", &["uid=-1".to_string()]).is_err());
    assert!(validator.validate_options("smb", &["port=0".to_string()]).is_err());
    assert!(validator.validate_options("smb", &["port=99999".to_string()]).is_err());
}
...
fn test_mode_validation_in_options() {
    ...
    assert!(validator.validate_options("sshfs", &["file_mode=0644".to_string()]).is_ok());
    assert!(validator.validate_options("sshfs", &["dir_mode=0755".to_string()]).is_ok());

    // Invalid mode values should fail
    assert!(validator.validate_options("sshfs", &["file_mode=7777".to_string()]).is_err());
    assert!(validator.validate_options("sshfs", &["file_mode=abc".to_string()]).is_err());
}
...
fn test_allowlist_enforcement() {
    ...
    for options in unsupported_options {
        let result = validator.validate_options("nfs", &options);
        assert!(result.is_err(), "Unsupported option should be rejected: {:?}", options);
    }
}
...
fn test_scheme_validation() {
    ...
    for scheme in allowed_schemes {
        ...
        assert!(validator.validate_url(&url).is_ok(), "Allowed scheme should pass: {}", scheme);
    }

    for scheme in blocked_schemes {
        let url = format!("{}://server.example.com/export", scheme);
        let result = validator.validate_url(&url);
        assert!(result.is_err(), "Blocked scheme should be rejected: {}", scheme);
        let error_msg = format!("{}", result.unwrap_err());
        assert!(
