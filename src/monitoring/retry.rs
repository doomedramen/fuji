...
impl RetryHandler {
    ...
    async fn get_policy(&self, mount_id: &str) -> RetryPolicy {
        let policies = self.retry_policies.read().await;
        policies.get(mount_id).cloned().unwrap_or_else(|| self.default_policy.clone())
    }
    ...
    async fn get_circuit_breaker(&self, mount_id: &str) -> CircuitBreakerState {
        ...
        if !breakers.contains_key(mount_id) {
            ...
            breakers.insert(mount_id.to_string(),
        ...
        Fut: std::future::Future<Output = Result<T>>,
    {
        ...
        loop {
            ...
            if !circuit_breaker.should_allow_attempt() {
                error!("Circuit breaker open for mount {}, blocking operation", mount_id);
            ...
            match operation().await {
                Ok(value) => {
                    ...
                    debug!("Operation succeeded for {} after {} attempts in {:?}",
                ...
                Err(e) => {
                    ...
                    if attempts >= policy.max_attempts {
                        error!("Max retry attempts ({}) exceeded for mount {}",
                    ...
                    if let Some(delay) = backoff.next_backoff() {
                        warn!("Attempt {} failed for mount {}, retrying in {:?}: {}",
...
mod tests {
    ...
    async fn test_retry_handler_success() {
        ...
        let result = handler.execute_with_retry("test", || async {
            Ok("success")
        }).await.unwrap();
