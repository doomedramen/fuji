...
impl JWTAuthenticator {
    ...
    pub fn validate_token(&self, token: &str) -> Result<FujiClaims> {
        ...
        {
            let revoked = self.revoked_tokens.try_read()
...
fn validate_token_structure(token: &str) -> Result<jsonwebtoken::TokenData<serde_json::Value>> {
    ...
    let claims: serde_json::Value = serde_json::from_slice(&decoded)
        .map_err(|_| anyhow!("Invalid claims format"))?;

    Ok(jsonwebtoken::TokenData { header: Header::default(), claims })
}
...
mod tests {
    ...
    fn test_jwt_generation_and_validation() {
        ...
        // Generate token
        let token = auth.generate_token("testuser", mounts.clone(), roles.clone()).unwrap();
    ...
    async fn test_token_revocation() {
        ...
        let token = auth.generate_token("user", HashSet::new(), HashSet::new()).unwrap();
