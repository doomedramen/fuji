...
impl PermissionManager {
    ...
    pub fn get_user_mapping(&mut self, username: &str) -> Result<UserMapping> {
        ...
        self.user_cache.insert(username.to_string(), mapping.clone());
        Ok(mapping)
    }
    ...
    fn lookup_user(&self, username: &str) -> Result<UserMapping> {
        ...
        let passwd_file = File::open("/etc/passwd")
            .map_err(|e| anyhow!("Failed to open /etc/passwd: {}", e))?;
    ...
    pub fn create_mount_point<P: AsRef<Path>>(
        ...
        let (final_uid, final_gid, final_mode) = if config.inherit && path.parent().is_some() {
            ...
            if parent.exists() {
                ...
                let uid = config.owner_uid.unwrap_or_else(|| Uid::from_raw(metadata.uid()));
                let gid = config.owner_gid.unwrap_or_else(|| Gid::from_raw(metadata.gid()));
        ...
        info!("Created mount point {} with uid:{} gid:{} mode:{:o}",
              path.display(), final_uid, final_gid, final_mode);
...
mod tests {
    ...
    fn test_create_mount_point() {
        ...
        manager.create_mount_point(&mount_path, Some(&config)).unwrap();
